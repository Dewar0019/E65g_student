...
@IBDesignable class GridView: UIView {
  ...
  var gridDataSource: GridProtocol?
  ...
  override func draw(_ rect: CGRect) {
    ...
    (0 ..< size+1).forEach { i in
      (0 ..< size+1).forEach { j in
        ...
        fillCircleColor(subRect, (gridDataSource?[i,j])!)
      }
    }
  }
  ...
  func processTouch(touches: Set<UITouch>) -> GridPosition? {
  ...
  func convertToPosition(touch: UITouch) -> GridPosition {
